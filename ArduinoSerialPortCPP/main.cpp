...
int main(int argc, char **argv)
{
  ...
  if (argc<3)
  {
      cerr << "No data to write is given" << endl;
      return 1;
  }
  else
      writtenData = argv[2];

  cout << "Serial port: " << portName << endl;
  cout << "Opening..." << endl;
  ...
  cout << "String sent------> " << writtenData << endl;
  serial_port.Write(writtenData);
  ...
  try
  {
      serial_port.Read(buffer,writtenData.size(),500);
  }
